...
static void dke2_xof_squeezeblocks(uint8_t *out,
    ...
    uint8_t input[DKE2_SEEDBYTES + 2 + 4];
    uint8_t block[32];
...
void DKE2_gen_matrix(polyvec *res,
    ...
    for (unsigned int i = 0; i < DKE2_K; ++i) {
        for (unsigned int j = 0; j < DKE2_K; ++j) {
            if (transposed) {
                dke2_sample_poly(&res[i].vec[j], seed, (uint8_t)i, (uint8_t)j);
            }
            else {
                dke2_sample_poly(&res[i].vec[j], seed, (uint8_t)j, (uint8_t)i);
            }
        }
    }
}
...
// We use the same table as long as we work in the same ring Rq
const int16_t DKE2_zetas[128] = {
...
static int16_t fqmul(int16_t a, int16_t b) {
    return DKE2_montgomery_reduce((int32_t)a * b);
}
...
void DKE2_invntt(int16_t r[256]) {
    ...
    for (len = 2; len <= 128; len <<= 1) {
        for (start = 0; start < 256; start = j + len) {
            ...
            for (j = start; j < start + len; j++) {
                t = r[j];
                r[j] = DKE2_barrett_reduce(t + r[j + len]);
                r[j + len] = r[j + len] - t;
...
void DKE2_basemul(int16_t r[2], const int16_t a[2], const int16_t b[2], int16_t zeta) {
    r[0]  = fqmul(a[1], b[1]);
    r[0]  = fqmul(r[0], zeta);
    r[0] += fqmul(a[0], b[0]);
    r[1]  = fqmul(a[0], b[1]);

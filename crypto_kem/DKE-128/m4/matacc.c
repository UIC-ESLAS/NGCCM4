...
*              - unsigned char i:            byte to indicate the index < DKE1_K of the row of A or A^T
...
void matacc_cache32(poly* r, const polyvec *b, polyvec *b_prime, unsigned char i, const unsigned char *seed, int transposed) {
  unsigned char buf[DKE1_XOF_BLOCKBYTES+2];
  dke1_xof_state state;
  int16_t c[4];
  int32_t r_tmp[DKE1_N]; // stores intermediate accumulated values to save reductions
  int j = 0;

  // 16-32

  if (transposed)
    dke1_xof_absorb(&state, seed, i, j);
  else
    dke1_xof_absorb(&state, seed, j, i);

  dke1_xof_squeezeblocks(buf, 1, &state);

  matacc_asm_cache_16_32(r_tmp, b->vec[j].coeffs, c, buf, zetas, &state, b_prime->vec[j].coeffs);

  // 32-32 DKE1_K - 2 times
  for(j=1;j<DKE1_K - 1;j++) {

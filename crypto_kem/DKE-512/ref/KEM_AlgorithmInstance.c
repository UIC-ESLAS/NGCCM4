...
#endif
	// SCHEME -------------------------------------------
	DKEM512_KeyGen(pk,sk,coins);
	*pk_len_bytes = DKE3_PKBYTES;
	*sk_len_bytes = DKE3_SKBYTES;
	return 0;
}
...
#endif
	// SCHEME -------------------------------------------
	DKEM512_Internal(ct, ss, pk, coins);
	*ss_len_bytes = DKE3_SSBYTES;
	*ct_len_bytes = DKE3_CTBYTES;
	return 0;
}

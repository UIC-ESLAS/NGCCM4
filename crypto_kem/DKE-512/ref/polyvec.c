...
void DKE3_polyvec_add(polyvec *res, const polyvec *a, const polyvec *b) {
    unsigned int i;
    for (i = 0; i < DKE3_K; i++) {
        DKE3_poly_add(&res->vec[i], &a->vec[i], &b->vec[i]);
    }
}
...
void DKE3_polyvec_ntt(polyvec *v) {
    unsigned int i;
    for (i = 0; i < DKE3_K; i++) {
        DKE3_poly_ntt(&v->vec[i]);
    }
}

void DKE3_polyvec_invntt_tomont(polyvec *v) {
    unsigned int i;
    for (i = 0; i < DKE3_K; i++) {
        DKE3_poly_invntt_tomont(&v->vec[i]);
    }
}

void DKE3_polyvec_basemul_acc_montgomery(poly *res, const polyvec *a, const polyvec *b) {
    unsigned int i;
    // the sums are accumulated in res
    poly temp;           // auxiliary poly to contain the successive products
    DKE3_poly_basemul_montgomery(res, &a->vec[0], &b->vec[0]);
    for (i = 1; i < DKE3_K; i++) {
        DKE3_poly_basemul_montgomery(&temp, &a->vec[i], &b->vec[i]);
        DKE3_poly_add(res, res, &temp);
    }
    DKE3_poly_reduce(res);
}
...
void DKE3_polyvec_tobytes(uint8_t bytes[DKE3_POLYVECBYTES], const polyvec *v) {
    unsigned int i;
    for (i = 0; i < DKE3_K; i++) {
        DKE3_poly_tobytes(bytes +  i * DKE3_POLYBYTES, &v->vec[i]);
    }
}
...
void DKE3_polyvec_frombytes(polyvec *v, const uint8_t bytes[DKE3_POLYVECBYTES]) {
    unsigned int i;
    for (i = 0; i < DKE3_K; i++) {
        DKE3_poly_frombytes(&v -> vec[i], bytes + i * DKE3_POLYBYTES);
    }
}
...
void DKE3_polyvec_compress11(uint8_t bytes[DKE3_PBCOMPRESSEDBYTES], const polyvec *v) {
    ...
    uint16_t t[8];
    for (i = 0; i < DKE3_K; i++) {
        for (j = 0; j < DKE3_N / 8; j++) { // we process 8 coefficients each time
            for (k = 0; k < 8; k++) {
                t[k]  = v->vec[i].coeffs[8 * j + k];
                t[k] += ((int16_t)t[k] >> 15) & DKE3_Q; // branch-free map to {0,...,q-1}
                // Compress from Zq --> Z2^d definition: round(x · 2^d / q) = floor(x · 2^d / q + 0.5)
                d0 = t[k]; // x
                d0 <<= 11; // x · 2^d
                d0 += 3840;  // x · 2^d + (q/2+0.5)
                d0 *= 279584;   // (2^31 / q) => d0 = (x·2^d + q/2 + 0.5)·(2^31/q)
...
void DKE3_polyvec_decompress11(polyvec *v, const uint8_t bytes[DKE3_PBCOMPRESSEDBYTES]) {
    ...
    for (i = 0; i < DKE3_K; i++) {
        for (j = 0; j < DKE3_N / 8; j++) {
            ...
            // Round(t q / 2^11)
            for (k = 0; k < 8; k++) {
                v->vec[i].coeffs[8 * j + k] = ((uint32_t)(t[k] & 0x7FF) * DKE3_Q + 1024) >> 11;
            }
        }
    }
}
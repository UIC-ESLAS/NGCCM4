...
void DKE512_Initiate(uint8_t pk[DKE3_PKBYTES],
    ...
    for (i = 0; i < DKE3_K; i++) {
        DKE3_polyvec_basemul_acc_montgomery(&pA.vec[i], &mat[i], &sA);
        DKE3_poly_tomont(&pA.vec[i]);
    }

    DKE3_polyvec_add(&pA, &pA, &eA);
    DKE3_polyvec_reduce(&pA);

    // WARNING: WE CAN COMMUNICATE pA DIRECTLY ON NTT DOMAIN BECAUSE WE ARE NOT ROUNDING HERE!

    DKE3_packpk(pk, &pA, seed);     // pk = (pA || seed) where pA = (1/2)A sA + eA (in NTT (Mont) domain)
    DKE3_CPA_packsk(sk, &sA);       // sk = sA (in NTT (Mont) domain)
}
...
void DKE512_Response(uint8_t ct[DKE3_CPA_CTBYTES],
    ...
    for (i = 0; i < DKE3_K; i++) {
        DKE3_polyvec_basemul_acc_montgomery(&pB.vec[i], &matt[i], &sB);
    }
    DKE3_polyvec_invntt_tomont(&pB); // exits from NTT
    DKE3_polyvec_add(&pB, &pB, &eB);   // pB = (1/2)A^tsB + eB
    DKE3_polyvec_reduce(&pB);

    // Arithmetic (computing kB) -------------------------------------------------------------------
    DKE3_polyvec_basemul_acc_montgomery(&kB, &pA, &sB);
    DKE3_poly_invntt_tomont(&kB);      // Exit NTT domain

...
void DKE3_polyvec_sub(polyvec *res, const polyvec *a, const polyvec *b) {
    unsigned int i;
    for (i = 0; i < DKE3_K; i++) {
...
void DKE3_polyvec_invntt(polyvec *v) {
    unsigned int i;
    for (i = 0; i < DKE3_K; i++) {
        DKE3_poly_invntt(&v->vec[i]);
    }
}

void DKE3_polyvec_basemul_acc(poly *res, const polyvec *a, const polyvec *b) {
    unsigned int i;
    DKE3_poly_basemul(res, &a->vec[0], &b->vec[0]);
    for (i = 1; i < DKE3_K; i++)
    {
        DKE3_poly_basemul_acc(res, &a->vec[i], &b->vec[i]);
    }

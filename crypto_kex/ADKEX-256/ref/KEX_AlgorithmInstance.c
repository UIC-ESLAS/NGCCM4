...
unsigned long long kex_get_passes_num()           { return ADKEX2_PASSES_NUM; }
unsigned long long kex_get_pk_len_bytes()         { return ADKEX2_PKBITS / 8; }
unsigned long long kex_get_sk_len_bytes()         { return ADKEX2_SKBITS / 8; }
unsigned long long kex_get_sta_len_bytes()        { return ADKEX2_STA_MAX_BITS / 8; }
unsigned long long kex_get_stb_len_bytes()        { return ADKEX2_STB_MAX_BITS / 8; }
unsigned long long kex_get_ss_len_bytes()         { return ADKEX2_SSBITS / 8; }
unsigned long long kex_get_total_msg_len_bytes()  { return ADKEX2_TOTAL_MSG_BITS / 8; }
...
{
    uint8_t coins[ADKEX2_INIT_B_COINBITS / 8];
    (void)stb;

    get_random_number(&drng_algorithm, coins, ADKEX2_INIT_B_COINBITS);
    ADKEX256_init_b_derand(pkb, skb, coins);

    *pkb_len_bytes = ADKEX2_PKBITS / 8;
    *skb_len_bytes = ADKEX2_SKBITS / 8;
...
{
    uint8_t coins[ADKEX2_PASS1_COINBITS / 8];
    (void)ska; (void)ska_len_bytes; (void)pkb_len_bytes;

    get_random_number(&drng_algorithm, coins, ADKEX2_PASS1_COINBITS);
    ADKEX256_pass1_msg_a_derand(m1, sta, pkb, coins);

    *m1_len_bytes  = ADKEX2_M1_BITS      / 8;
    *sta_len_bytes = ADKEX2_STA_MAX_BITS / 8;
    ...
    unsigned char *m2,  unsigned long long *m2_len_bytes)
{
    ...
    uint8_t coins[ADKEX2_PASS2_COINBITS / 8];
    const uint8_t *pk_B = skb + (ADKEX2_SKBITS - ADKEX2_PKBITS - ADKEX2_SSBITS) / 8;

    (void)skb_len_bytes; (void)pka; (void)pka_len_bytes;
    (void)m1_len_bytes;

    get_random_number(&drng_algorithm, coins, ADKEX2_PASS2_COINBITS);
    ADKEX256_pass2_msg_b_derand(m2, stb, m1, pk_B, skb, coins);

    *m2_len_bytes  = ADKEX2_M2_BITS      / 8;
    *stb_len_bytes = ADKEX2_STB_MAX_BITS / 8;
    return 1;                            /* last pass */
}
    ...
    unsigned char *ssa, unsigned long long *ssa_len_bytes)
{
    ...
    ADKEX256_derive_ss_a(ssa, mb, sta, pkb);
    *ssa_len_bytes = ADKEX2_SSBITS / 8;
    return 0;
}
    ...
    unsigned char *ssb, unsigned long long *ssb_len_bytes)
{
    ...
    ADKEX256_derive_ss_b(ssb, stb);
    *ssb_len_bytes = ADKEX2_SSBITS / 8;
    return 0;
}
